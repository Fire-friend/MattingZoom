...
def select_roi(uncertain_map, mode='hist'):
    with torch.no_grad():
        roi = []
...
def get_trimap2(alpha):
    alpha_fg = alpha.copy()
    alpha_fg_erode = alpha.copy()
    alpha_fg[alpha_fg < 1] = 0
    alpha_fg_erode[alpha_fg_erode > 0] = 255
    ...
    # iterations = 1
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k_size, k_size))
    dilated = cv2.dilate(alpha_fg, kernel, iterations)
    eroded = cv2.erode(alpha_fg_erode, kernel, iterations)
    # cv2.imshow('dilated', dilated)
    # cv2.imshow('eroded', eroded)
    # cv2.imshow('dis', dilated - eroded)
    # cv2.waitKey(500)
