...
##### U^2-Net ####
class U2Net_Net(nn.Module):

    def __init__(self, args):
        super(U2Net_Net, self).__init__()
        in_ch = args.in_ch
        out_ch = args.out_ch
    ...
    def forward(self, x, mode='train'):
        ...
        if mode == 'train':
            ...
            return F.sigmoid(d0), F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d5), F.sigmoid(
                d6)
        # return F.sigmoid(6), F.sigmoid(d5), F.sigmoid(d4), F.sigmoid(d3), F.sigmoid(d2), F.sigmoid(d1), F.sigmoid(d0)
