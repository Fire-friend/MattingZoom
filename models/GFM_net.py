...
class GFM_Net(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.backbone == 'r34':
            ...
            # self.psp_module_ae = PSPModule(512, 512, (1, 3, 5))
            # self.psp4_ae = conv_up_psp(512, 256, 2)
            # self.psp3_ae = conv_up_psp(512, 128, 4)
            # self.psp2_ae = conv_up_psp(512, 64, 8)
            # self.psp1_ae = conv_up_psp(512, 64, 16)

            # self.decoder4_g_ae = build_decoder(1024, 512, 512, 256, True, True)
            # self.decoder3_g_ae = build_decoder(512, 256, 256, 128, True, True)
            # self.decoder2_g_ae = build_decoder(256, 128, 128, 64, True, True)
            # self.decoder1_g_ae = build_decoder(128, 64, 64, 64, True, True)
            ...
            if self.rosta == 'RIM':
                ...
                # self.decoder0_g_ae = build_decoder(128, 64, 64, self.gd_channel, False, True)
                # self.decoder1_g_ae_res = build_decoder(64, 64, 64, self.gd_channel, False, False)
                # self.decoder2_g_ae_res = build_decoder(64, 64, 64, self.gd_channel, False, False)
                # self.decoder3_g_ae_res = build_decoder(128, 64, 64, self.gd_channel, False, False)
    ...
    def forward(self, input):
        ...
        #     d4_g = self.decoder4_g(torch.cat((self.psp5(psp), d5_g), 1))
        # else:
        #     psp_ae = self.psp_module_ae(e4)
        #     d4_g_ae = self.decoder4_g_ae(torch.cat((psp_ae, e4), 1))
        #
        # d3_g_ae = self.decoder3_g_ae(torch.cat((self.psp4_ae(psp_ae), d4_g_ae), 1))
        # res3 = self.decoder3_g_ae_res(d3_g_ae)
        # d2_g_ae = self.decoder2_g_ae(torch.cat((self.psp3_ae(psp_ae), d3_g_ae), 1))
        # res2 = self.decoder2_g_ae_res(d2_g_ae)
        # d1_g_ae = self.decoder1_g_ae(torch.cat((self.psp2_ae(psp_ae), d2_g_ae), 1))
        # res1 = self.decoder1_g_ae_res(d1_g_ae)
        #
        # if self.backbone == 'r34_2b':
        #     if self.rosta == 'RIM':
        #         d0_g_tt = self.decoder0_g_tt(d1_g)
        ...
        #         d0_g_bt = self.decoder0_g_bt(torch.cat((self.psp1(psp), d1_g), 1))
        #     else:
        #         d0_g_ae = self.decoder0_g_ae(torch.cat((self.psp1_ae(psp_ae), d1_g_ae), 1))
        ...
        if self.rosta == 'RIM':
            ...
            return glance_sigmoid, focus_sigmoid, fusion_sigmoid#, d0_g_ae, res1, res2, res3
